...
import { BOXING_COMBOS } from "./boxing";
...
/* --------------- Spoken combo announcements (phrase clips) --------------- */

// A combo is announced by stringing together short pre-generated word clips —
// "up next" + one clip per name segment ("Jab", "Lead Hook", …). The clips are
// generated by scripts/genVoice.mjs into /public/voice/<slug>.mp3.

function phraseSlug(s: string): string {
  return s
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
...
const phraseFile = (slug: string) => `/voice/${slug}.mp3`;

const phraseBuffers: Partial<Record<string, AudioBuffer>> = {};
let phraseLoadStarted = false;

async function loadPhraseBuffers(): Promise<void> {
  const c = graph();
  if (!c || phraseLoadStarted) return;
  phraseLoadStarted = true;
  await Promise.all(
    PHRASE_SLUGS.map(async (slug) => {
      try {
        const res = await fetch(phraseFile(slug));
        if (!res.ok) return;
        phraseBuffers[slug] = await c.decodeAudioData(await res.arrayBuffer());
      } catch {
        /* clip missing — fall back to an <audio> element at play time */
      }
    }),
  );
}

// Sources for the announcement currently playing, so it can be cancelled.
...
export function speakCombo(name: string): void {
  ...
  // Preferred path: schedule the decoded clips back-to-back on the audio clock.
  if (c && voiceGain && buffers.every(Boolean)) {
    let when = c.currentTime + 0.04;
    for (const buf of buffers as AudioBuffer[]) {
      const src = c.createBufferSource();
      src.buffer = buf;
      src.connect(voiceGain);
      comboSources.add(src);
      src.onended = () => comboSources.delete(src);
      src.start(when);
      when += buf.duration + 0.06; // brief gap for natural cadence
    }
    return;
  }
