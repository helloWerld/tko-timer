...
const spot = [
  ["1 - 2 - 3", { punches: 3, difficulty: "beginner", hasSlipBlock: false, hasDuck: false, hasFootwork: false }],
  ["1 - 2 - 3 - 4", { punches: 4, difficulty: "intermediate" }],
  ["2 - 3 - 2 - 3 - 2", { punches: 5, difficulty: "advanced" }],
  ["SL - 2", { punches: 1, difficulty: "beginner", hasSlipBlock: true }],
  ["1 - 2 - DL - 4", { punches: 3, difficulty: "beginner", hasDuck: true }],
  ["1 - 2 - 3 - PL", { punches: 3, hasFootwork: true }],
  ["1 - 4B", { punches: 2, difficulty: "beginner" }],
  ["BL - 2 - 3 - 2 - 3", { punches: 4, difficulty: "intermediate", hasSlipBlock: true }],
];
...
console.log(`  difficulty distribution: ${JSON.stringify(dist)}`);
...
for (const level of ["beginner", "intermediate", "advanced"]) {
  const cap = { beginner: 3, intermediate: 4, advanced: 99 }[level];
  const pool = boxingComboPool(level, allOn);
  const overCap = pool.filter((c) => c.punches > cap);
  if (overCap.length) fail(`${level}: ${overCap.length} combos exceed punch ceiling ${cap}`);
...
if (!failures) ok("pools respect ceiling and element toggles");
    ...
    for (const intensity of ["low", "medium", "high"]) {
      for (const t of toggles) {
        for (const targetMinutes of [5, 20, 45]) {
          ...
          const cap = { beginner: 3, intermediate: 4, advanced: 99 }[difficulty];
...
if (!failures) ok("all generated workouts satisfy invariants");
